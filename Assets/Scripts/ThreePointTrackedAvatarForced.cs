...
namespace Ubiq.Avatars
{
    ...
    public class ThreePointTrackedAvatarForced : MonoBehaviour
    {
        ...
        private void Update ()
        {
            if(avatar.IsLocal)
            {
                // AJS temporary workaround to allow OpenVR avatar to set positions, without creating a normal avatar
                ...
                state[0].head = GetPosRot(head);
                state[0].leftHand = GetPosRot(lhand);
                state[0].rightHand = GetPosRot(rhand);
                state[0].leftGrip = 0f;
                state[0].rightGrip = 0f;
        ...
        private PositionRotation GetPosRot(GameObject obj)
        {
            var posrot = PositionRotation.identity;

            posrot.position = obj.transform.position;
            posrot.rotation = obj.transform.rotation;
